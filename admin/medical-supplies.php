...
<div class="container-fluid px-4 py-4">
  <div class="d-flex justify-content-between align-items-center mb-4">
    <h2 class="fw-semibold mb-0">Medical Supplies</h2>

    <div class="d-flex align-items-center gap-2">
      <input type="text" id="search-inventory" class="form-control" placeholder="Search inventory..." style="width: 250px;">
  ...
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content rounded-4">
      <div class="modal-header border-0">
        <h5 class="modal-title fw-semibold text-primary" id="inventoryModalLabel">Add / Edit Item</h5>
...
<script>
  (function() {
    ...
    function fetchInventory() {
      axios.get('/healthsync/api/inventory/medical-supplies.php')
        .then(res => {
          const medicineTable = $('#medicine-table-body').empty();
          ...
          (res.data || []).forEach(item => {
            ...
            tr.append(`<td>${item.expiration_date}</td>`);
        ...
        .catch(() => showToast('Failed to load inventory.', 'danger'));
    }
    ...
    $(document).ready(function() {
      ...
      $('#inventoryModal').on('hidden.bs.modal', clearForm);
      ...
      $('#save-inventory').click(function() {
        ...
        formData.append('expiration-date', $('#expiration-date').val());
        ...
        axios.post('/healthsync/api/inventory/medical-supplies.php', formData)
          .then(() => {
            $('#inventoryModal').modal('hide');
            fetchInventory();
            showToast(id ? 'Item updated successfully!' : 'Item added successfully!');
          })
          .catch(() => showToast('Error saving item.', 'danger'));
      });


      $(document).on('click', '.edit-btn', function() {
        const id = $(this).data('id');
        axios.get(`/healthsync/api/inventory/medical-supplies.php?id=${id}`)
          .then(res => {
            const item = res.data;
            ...
            $('#expiration-date').val(item.expiration_date)
      ...
      $('#confirm-delete-btn').click(function() {
        axios.delete('/healthsync/api/inventory/medical-supplies.php', {
          data: {
            id: deleteId
          }
        }).then(() => {
    ...
    $('#search-inventory').on('keyup', function() {
      ...
      $('#medicine-table-body tr').each(function() {
