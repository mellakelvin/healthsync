...
<script>
  (function() {
    ...
    async function loadChartData(range, chart) {
      try {
        const response = await fetch(`/healthsync/api/appointment/stats.php?range=${range}&type=labtech`);
  ...
  `;
    ...
    function loadTestTypeChart(type = null) {
      axios.get('/healthsync/api/appointment/test-type-stats.php?type=labtech')
